...
def custom_init(device, wav2vec): # 初始化音频模型
    audio_encoder = Wav2Vec2Model.from_pretrained(wav2vec, local_files_only=True).to(device) # 自定义模型加载
    audio_encoder.feature_extractor._freeze_parameters() # 冻结特征提取器的参数
    wav2vec_feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(wav2vec, local_files_only=True) # 官方 Hugging Face 特征提取器
    return wav2vec_feature_extractor, audio_encoder # 输出：特征提取器和音频编码器

def loudness_norm(audio_array, sr=16000, lufs=-23): # 音频信号响度标准化
    meter = pyln.Meter(sr)
...
def audio_prepare_multi(left_path, right_path, audio_type, sample_rate=16000): # 处理多人音频
    ...
    elif audio_type=='add': # 音频顺序只能是先左后右
...
def get_embedding(speech_array, wav2vec_feature_extractor, audio_encoder, sr=16000, device='cpu'): # 得到音频特征
    ...
    # audio encoder
    with torch.no_grad():
        embeddings = audio_encoder(audio_feature, seq_len=int(video_length), output_hidden_states=True) # 对特征序列进行线性插值，保证长度和视频帧数对齐
...
def extract_audio_from_video(filename, sample_rate): # 处理视频的音频
...
def audio_prepare_single(audio_path, sample_rate=16000): # 处理音频逻辑
    ext = os.path.splitext(audio_path)[1].lower()
    if ext in ['.mp4', '.mov', '.avi', '.mkv']:
        human_speech_array = extract_audio_from_video(audio_path, sample_rate)
        return human_speech_array
    else:
        human_speech_array, sr = librosa.load(audio_path, sr=sample_rate) # 音频重采样
        human_speech_array = loudness_norm(human_speech_array, sr) # 音频响度归一化
...
def generate(args):
    ...
    if args.ulysses_size > 1 or args.ring_size > 1:
        ...
        initialize_model_parallel( # 初始化模型上下文并行
    ...
    #     if args.prompt_extend_method == "dashscope": # 是否启用提示词扩展功能
    ...
    assert args.task == "multitalk-14B", 'You should choose multitalk in args.task.' # 限制任务类型
    

    # TODO: add prompt refine
    # img = Image.open(args.image).convert("RGB") # 多模态任务的视觉输入
    # if args.use_prompt_extend: # 是否启用图片提示词扩展功能
